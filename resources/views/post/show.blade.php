                ...
                <div class="relative inline-block text-left">
                    <div class="w-32">
                        <button type="button"
                            ...
                            id="menu-button_delete" aria-expanded="true" aria-haspopup="true">
                    ...
                    <div class="absolute right-0 z-10 mt-2 w-32 origin-top-left rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none hidden"
                        role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                        <div class=" " role="none">
                            <form method="POST" action="{{ route('post.destroy', $post )}}" class="m-1 justify-center">
                                @method('DELETE')
                                @csrf
                                <input value="{{__('traslate.posts.dpost')}}"
                                    class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded cursor-pointer text-sm"
                                    type="submit" />
                            </form>
                        </div>
                    </div>
    ...
    <div class="md:w-1/2 flex-col ">
        <div class="shadow-lg  p-5 mb-5 rounded-md bg-comentary">
            <div class="w-8/12 lg:w-6/12 sm:flex justify-items-start flex justify-start pb-5">
                ...
                @else
                <img class="rounded-full w-20 h-20 border-4 object-cover object-top border-gray-900"
                    src="{{ asset('perfiles/' . $user->imagen_perfil) }}">
                @endif
                <span
                    class="post-user font-bold text-3xl flex justify-center items-center pl-5 color-comentari">{{$post->user->username}}</span>
            </div>

            <hr>
            <div class="mb-5">
                <p class="post-user text-xl font-bold pt-4 color-comentari">{{__('traslate.posts.description')}}</p>
                <p class="post-user mt-5 max-h-28 overflow-y-scroll scrollbar-thin color-comentari">
                    {{$post->descripcion}}</p>
            </div>
            <hr>
            <div class="flex">
                <div class="w-1/2 mt-3">
                    <div class="flex items-center">
                        ...
                        <div class="flex gap-2 items-center">
                            ...
                            <p class="post-user color-comentari"> {{$post->comentarios->count()}}</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="mb-5 max-h-96 overflow-y-scroll scrollbar-thin">
                ...
                <div class=" border-gray-300 ">
                    <div class="flex">
                        ...
                        <img class="rounded-full w-16 h-16 object-cover object-top border-2 border-gray-900"
                            src="{{ asset('perfiles/' . $comentario->user->imagen_perfil) }}" alt="">
                        @else
                        <img class="rounded-full w-16 h-16" src="{{ asset('img/usuario.svg') }}" alt="">
                        @endif

                        <div class="flex-1">
                            <span class="pl-2 comentarios">
                                <a href="{{ route('post.index', $comentario->user) }}"
                                    class="font-bold color-comentari">{{
                                    str_replace('-', ' ',
                                    $comentario->user->username)}}</a>
                            </span>
                            <p class="pl-2 comentarios color-comentari">ﾠ{{ $comentario->comentario }}</p>
                        </div>
                    </div>

                    <p class="text-sm color-comentari flex justify-end items-end">{{
            ...
            <form action="{{ route('comentarios.store', ['post' => $post, 'user' => $user]) }}" method="POST">
                ...
                <div class="relative mb-3">
                    <label for="comentario"
                        class="mb-2 block uppercase color-comentari font-bold">{{__('traslate.posts.addcomment')}}</label>
                    <textarea id="comentario" name="comentario"
                        class="border p-3 w-full rounded-lg @error('name') border-red-500 @enderror"
                        placeholder="{{__('traslate.posts.pdescription')}}">{{ old('comentario')}}</textarea>
                    @error('comentario')
                    <p class="text-red-700">{{ $message }}</p>
                    @enderror
                </div>

                <input type="submit" value="{{__('traslate.posts.addcomment')}}"
...
<div class="w-full flex flex-col items-center md:flex-row">
    <div class="w-8/12 lg:w-6/12 sm:flex sm:justify-items-center md:flex md:justify-end">
        ...
        @else
        <img class="rounded-full w-60 h-60 object-cover object-top border-2 border-gray-900"
            src="{{ asset('perfiles/' . $user->imagen_perfil) }}">
        @endif
    </div>
    ...
    <div class="md:w-8/12 lg:w-6/12 px-5 flex flex-col items-center md:justify-center md:items-start py-10 md:py-10">
        ...
        <div class="pt-4">
            <form action="{{ route('users.follow', $user)}}" method="POST">
                @csrf
                <input type="submit"
                    class="bg-gray-500 transition duration-300 ease-in-out hover:bg-blue-500 text-white uppercase rounded-lg px-3 py-1 text-lg font-bold cursor-pointer"
                    value="{{__('traslate.follow.follow')}}">
            </form>
        </div>
        @else
        <div class="pt-4">
            <form action="{{ route('users.unfollow', $user)}}" method="POST">
                ...
                @method('DELETE')
                <input type="submit"
                    class="bg-gray-500 transition duration-300 ease-in-out hover:bg-red-500 text-white uppercase rounded-lg px-3 py-1 text-lg font-bold cursor-pointer"
                    value="{{__('traslate.follow.unfollow')}}">
            </form>
        </div>
        @endif
        {{-- agregamos esta condicional para que el boton no aparezca en la pagina de nosotros y solo se muestre en
        otros perfiles diferentes para para no auto seguirnos --}}
        @endif
    </div>
</div>
<div class="flex justify-center items-center rounded-lg">
    {!! trans('traslate.follow.message.imagen_follow_no_follow') !!}
    <!-- Esto mostrará la imagen de seguir -->
</div>
@endsection
...
<div class="flex justify-center items-center rounded-lg">
    {!! trans('traslate.follow.message.imagen__no_follow_follow') !!}
    <!-- Esto mostrará la imagen de seguir -->
</div>
@endsection
...
<div class="flex justify-center items-center rounded-lg">
    {!! trans('traslate.follow.message.imagen_no_follow') !!}
    <!-- Esto mostrará la imagen de seguir -->
</div>
@endsection
            ...
            <form action="{{ route('comentarios.store', ['post' => $post, 'user' => $user]) }}" method="POST">
                ...
                <div class="relative mb-3">
                    ...
                    <textarea id="comentario" name="comentario"
                        class="border p-3 w-full rounded-lg @error('name') border-red-500 @enderror"
                        placeholder="{{__('traslate.posts.pdescription')}}">{{ old('comentario')}}</textarea>
                    <span id="charCount" class="text-sm text-gray-500">255 caracteres restantes</span>
                    @error('comentario')
                    <p class="text-red-700">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                ...
                textarea.addEventListener('input', function () {
                    ...
                    charCountSpan.textContent = remainingChars + ' caracteres restantes';
